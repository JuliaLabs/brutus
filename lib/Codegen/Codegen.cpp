                             ...
                             mlir::Value value, jl_datatype_t *expected_type)
{
    ...
    if (!jl_egal(value_type, (jl_value_t *)expected_type) && jl_subtype(value_type, (jl_value_t *)expected_type))
    {
                           ...
                           std::string name)
{
    ...
    for (int i = 0; i < (int)nstmts; i++)
    {
        ...
        mlir::Location loc = (linetable_index == 0) ? mlir::UnknownLoc::get(ctx.context) : locations[linetable_index - 1];
...
extern "C"
{
    ...
    mlir::ModuleOp brutus_codegen_jlir(mlir::MLIRContext *context,
                                        jl_value_t *methods,
    ...
    ExecutionEngineFPtrResult brutus_codegen(jl_value_t *methods, jl_method_instance_t *entry_mi, char emit_fptr, char dump_flags)
    {
        ...
        auto module = brutus_codegen_jlir(&context, methods, entry_mi);
